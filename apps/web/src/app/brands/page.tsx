...
export default async function BrandsPage() {
  ...
  return (
    <div className="shell pb-16">
      <Breadcrumbs items={[{ name: "Начало", href: "/" }, { name: "Марки", href: "/brands" }]} />
      <SectionHeading as="h1" title="Марките, които предлагаме" description="Разгледайте асортимента по производител." />

      {stocked.length === 0 ? (
        <EmptyState title="Още няма марки" description="Каталогът все още се синхронизира. Моля, проверете отново скоро." />
      ) : (
          ...
          {stocked.map((brand) => (
            <li key={brand.slug}>
              ...
              >
                ...
                {brand.tagline && <p className="mt-1 text-sm text-clay-600 italic">{brand.tagline}</p>}
                {brand.description && (
