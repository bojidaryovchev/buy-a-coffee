...
import { AvailabilityBadge, Badge, Breadcrumbs, ButtonLink, SectionHeading } from "@/components/ui/primitives";
      ...
      <div className="grid gap-10 lg:grid-cols-2 lg:gap-14">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2">
            ...
            {product.discountPercent !== null && <Badge tone="accent">−{product.discountPercent}%</Badge>}
          </div>

          <div className="mt-6 flex items-baseline gap-3">
            {product.price ? (
              <>
                <p className="font-display text-3xl font-semibold text-ink-900">{product.price.formatted}</p>
                {product.oldPrice && (
                  <p className="text-lg text-ink-300 line-through">{product.oldPrice.formatted}</p>
                )}
              </>
            ) : (
              <p className="text-lg text-ink-500">Цена при запитване — обадете ни се и ще ви я кажем.</p>
            )}
          </div>
            ...
            {unavailable ? (
              <div className="rounded-md border border-clay-500/40 bg-clay-100 p-5">
                ...
                <p className="mt-1 text-sm text-ink-700">
                  Спряхме да го предлагаме. Обадете ни се и ще ви насочим към най-близкото, което имаме.
                </p>
                <div className="mt-4 flex flex-wrap gap-3">
                  <ButtonLink href={primaryCategory ? `/categories/${primaryCategory.slug}` : "/categories"}>
                    Виж подобни кафета
                  </ButtonLink>
                  <a
              ...
              <QuickOrderForm productSlug={product.slug} disabled={product.availability === "out_of_stock"} />
            )}
          </div>
            ...
            <dl className="mt-8 divide-y divide-line border-y border-line text-sm">
              {attributeRows.map(([key, value]) => (
                <div key={key} className="flex justify-between gap-4 py-2.5">
                  ...
                  <dd className="text-right font-medium text-ink-900">{attributeValueLabel(key, value)}</dd>
                </div>
              ))}
              {product.sku && (
                  ...
                  <dd className="text-right">
                    {product.categories.map((category, index) => (
                      <span key={category.slug}>
                        {index > 0 && ", "}
                        <Link href={`/categories/${category.slug}`} className="font-medium text-pine-700 underline-offset-4 hover:underline">
                          {category.name}
                        </Link>
                      </span>
                    ))}
